...
### 관로 네트워크 분기·곡관 판정 및 진단 (③④⑤) (2026-07-30)
- [ ] `docs/20_관로네트워크_분기곡관_지시서.md`를 기준으로 다음 작업에서 구현 예정.
  - 범위: 곡관 판정(`BendToleranceTable`/`BendResolver`), 허용굴곡 DataStorage 설정,
    관로 네트워크 진단 UseCase·2탭 UI·Revit 명령·리본 등록, 회귀 테스트.
  - 제외: Revit 이형관/곡관 패밀리 배치, 기존 `PipeNetworkBuilder`/`PipeNetworkGraph` 공개 계약 변경.
- [ ] 시작 전 사용자 확인 필요: 표준각 임시 허용굴곡 기본값(관종 무관: 450mm 이하 5°, 800mm 이하 4°, 1500mm 이하 3°)은
      지시서에 따라 TODO 주석과 함께 구현하되, KWWA/KS 표 검증은 별도 확인 필요.
...
- [x] `Core/Gis/PipeNetworkBuilder.cs` 신규 — 여러 폴리선을 하나의 위상 그래프로 조립. 선분 쌍의 최근접점을
      허용오차로 판정해 **끝점-선분 접촉(T분기)과 선분-선분 교차(십자)를 같은 계산으로 처리**하고 양쪽을 분할한다.
      격자 해시로 후보 쌍을 추려 전수 비교를 피하고, 절점 병합도 격자 해시로 한다. 기본 스냅 허용오차 10mm.
- [x] `Core/Gis/PipeNetworkClassifier.cs` 신규 — 차수 + 직경 조합으로 `NodeKind`(EndPoint/Straight/Reducer/Bend/
      Tee/ReducingTee/Cross/TooManyBranches) 판정. 차수 2 절점의 편각을 3D 입체각으로 계산해 노출한다.
- [x] `tests/DHBIMWATER.UI.Tests/Gis/PipeNetworkBuilderTests.cs` 신규 10건 — T분기/이경티/십자/스냅 병합/
      허용오차 밖 미접속/입체교차 비분기/편각 45°/직선 0°/중복 정점/차수 5 경고.
- [x] 검증: `dotnet build src\DHBIMWATER.Core` 오류 0(기존 경고 7건). **테스트 프로젝트는 이번 작업과 무관한
      기존 컴파일 오류**(`QuantitySettingsConsumerTests`, `ViewModels/Quantity/*`가 없어진 타입 참조)로 실행 불가.
      대신 Core 소스를 직접 참조하는 임시 콘솔 러너로 위 10건 + 16° 편각 케이스까지 15개 검사 전부 PASS 확인.
- [x] `docs/20_관로네트워크_분기곡관_지시서.md` 작성 — 나머지 ③④⑤ 단계의 Codex 지시서.
- [ ] TODO: 평행 중첩 폴리선(같은 경로에 두 번 그려진 선)은 접촉점이 하나로 정해지지 않아 현재 분할 대상에서
      제외된다. 데이터 오류 진단 항목으로 별도 처리 필요 (`PipeNetworkBuilder.TryClosestParameters` 주석).
- [ ] TODO: 기존 테스트 프로젝트 컴파일 오류 복구(수량산출 타입 정리) — 별도 작업.

### 공기밸브실 Void 시작 면·배치점 재검토 (2026-07-30)
- [x] 실제 오류 `해당 호스트에서 void_면기반의 인스턴스(instance)를 절단할 수 없습니다.`를 배치 코드 기준으로 검토했다.
- [x] 원인: `RevitAirValveVoidCommandRepo`가 실제 선택한 `PlanarFace`가 아닌 기초 `BoundingBox.Min.X/Min.Y`를 배치점으로 사용한다. 독립기초를 -90° 회전한 뒤에는 이 좌표가 선택된 호스트 면 위의 유효 점임을 보장하지 않아 Void가 호스트 밖에서 시작할 수 있다. 또한 `Axis`만으로는 RFA의 Void 돌출 로컬 방향에 대응하는 시작 면(-/+X, -/+Y)을 명시하지 못한다.
- [x] 수정: `void_면기반`의 Void가 호스팅 외곽면에서 기초 내부로 돌출한다는 기준으로 X/Y 모두 음수 방향 외곽면을 시작면으로 고정했다. 선택된 `PlanarFace`에 관통 중심을 투영한 실제 면 위 점을 `NewFamilyInstance`에 전달하며, 투영점의 Face 경계 내부 여부도 확인한다. 회전 후 BoundingBox 최소좌표는 더 이상 배치점으로 쓰지 않는다.
- [x] 검증: `dotnet build src\\DHBIMWATER.Infrastructure\\DHBIMWATER.Infrastructure.csproj --no-restore -p:DebugType=None` 오류 0 (기존 nullable·PDB 잠금 등 경고 152개).
- [x] 사용자 제공 `보이드.png` 확인: X축 요청에도 Void가 기초 측면을 관통하지 않고 면에 접선(Y축처럼 보이는) 방향으로 생성되며, 기초 외부까지 벗어난다. 배치점만의 문제가 아니라 `void_면기반` RFA의 로컬 Void 축과 API의 인스턴스 회전 기준이 맞지 않은 상태다.
- [ ] TODO: `void_면기반.rfa`의 실제 Void 돌출 축/원점과 `Cut with Voids When Loaded` 설정을 확인한 뒤, 이에 맞춰 호스트 면 및 `referenceDirection`을 확정한다. 현재 다운로드 폴더에는 RFA가 없고 이미지 파일만 확인됐다.

### 공기밸브실 Void 호스트 면 탐색 실패 수정 (2026-07-30)
- [x] 증상: "독립기초에서 X축 Void를 위한 측면을 찾을 수 없습니다."
- [x] 원인: 독립기초는 로드 패밀리 인스턴스라 `Element.get_Geometry()`의 최상위 멤버가 `Solid`가 아니라 `GeometryInstance`다. 기존 `FindPlanarFace`의 `.OfType<Solid>()`가 항상 비어 루프가 한 번도 돌지 않고 `null`을 반환했다. workplane 설정과는 무관하다.
- [x] 수정: `GeometryInstance.GetInstanceGeometry()`를 재귀 순회하는 `EnumeratePlanarFaces`를 추가해 실제 Solid/PlanarFace를 추출한다. (`RevitAirValveVoidCommandRepo.cs`)
- [x] 법선 일치 판정을 `XYZ.IsAlmostEqualTo`(허용오차 ~1e-9)에서 `DotProduct > 0.999`로 완화해 기초 -90° 회전의 부동소수 오차를 흡수한다.
- [x] 면 탐색 실패 시 실제 검출된 평면 법선 목록을 예외 메시지에 포함해 디버깅이 가능하게 했다.
- [x] `dotnet build src\DHBIMWATER.Infrastructure` 오류 0개(기존 경고 158개, VS PDB 잠금 경고 포함).
- [ ] TODO: Revit에서 실제 배치 검증. `void_면기반` 패밀리가 **면 기반(Face Based) 템플릿**으로 작성되어 있어야 `NewFamilyInstance(Reference, ...)` 호스팅이 동작한다.

### 공기밸브실 본관 관통 Void 입력·배치 (2026-07-30)
- [x] 공기밸브실 선택 시 기초 Toe 기본값을 100mm로 적용했다. 본관 직경, 기존 기초 두께, 기초 상부~본관 중심 깊이, 관통 축(X 기본/Y 선택)을 입력받으며, 공기밸브실에서는 버림콘크리트 입력·생성을 제외한다.
- [x] 프로젝트에 로드된 면기반 `void_면기반` 패밀리를 공기밸브실 독립기초의 X/Y 측면에 호스팅한다. 본관 중심 깊이로 표고를 지정하고 인스턴스 길이 매개변수 `r`에 본관 직경의 절반을 설정한다. 패밀리 미로드 또는 `r` 미설정 시 전체 생성 트랜잭션을 롤백하고 원인을 알린다.
- [x] 계산기 회귀 테스트에 버림콘크리트 제외, X/Y축별 Void 중심 위치, 반지름을 추가했다. Application 빌드 오류 0, Infrastructure 빌드 오류 0(기존 Revit 참조 경고 4개). UI 빌드 성공 후 테스트 프로젝트는 기존 수량산출 관련 누락 타입 컴파일 오류로 실행 불가했다.
- [ ] TODO: `void_면기반.rfa`를 프로젝트에 로드하고, 패밀리의 Void 돌출 깊이를 기초 반대편까지 충분히 설정한 상태에서 Revit에서 X/Y축 관통 결과를 확인한다.

### 공기밸브실 독립기초 자동 배치 보완 (2026-07-30)
- [x] 복제 독립기초 유형의 `STRUCTURAL_FOUNDATION_LENGTH`·`STRUCTURAL_FOUNDATION_WIDTH`를 공기밸브실 입력으로 갱신한다. 치수는 기존 기초 Floor와 동일하게 외벽 외곽 + 양측 기초 Toe(각 방향 `Toe × 2`)를 적용한다.
- [x] 회귀 테스트에서 길이 3,300mm·폭 3,000mm·두께 650mm 입력이 독립기초 정의에 전달되는지 검증한다.
- [x] 공기밸브실 독립기초 인스턴트를 배치점 중심으로 시계방향 90° 회전한다. 길이·폭 설정 방향을 밸브실 축(X=길이, Y=폭)에 맞춘다.

### 공기밸브실 하부 독립기초 전환 (2026-07-30)
- [x] 공기밸브실만 기존 `Floor` 기초를 생성하지 않고, 버림콘크리트는 기존처럼 Floor로 유지한다. 이토·제수밸브실의 기초 Floor 생성 경로는 변경하지 않았다.
- [x] 현재 문서의 첫 번째 독립기초 인스턴스를 복제하고, `공기밸브실 기초 - {두께}mm` 유형을 원본 유형에서 복제해 배정한다. 기존 UI의 `기초 두께` 사용자 입력값을 복제 유형의 `STRUCTURAL_FOUNDATION_THICKNESS`에 적용한다.
- [x] 독립기초 상단 중심을 공기밸브실 내부 중심(X=길이/2, Y=폭/2) 및 기초 상부 EL에 정렬했다. 원본 독립기초가 없거나 유형에 두께 파라미터가 없으면 트랜잭션을 롤백하고 원인을 알린다.
- [x] 회귀 테스트: 공기밸브실에서 Floor 기초가 제외되고 독립기초 정의가 입력 두께·중심·표고를 사용함을 추가. Application·Infrastructure 빌드 오류 0개(기존 PDB 잠금/Revit 참조 경고만 존재). UI 테스트 프로젝트는 기존 수량산출 테스트의 누락 타입 컴파일 오류로 실행 불가.

### 밸브실 내부 높이 기준 통일 (2026-07-30)
- [x] 제수·공기·이토밸브실(중간슬래브 없음)의 `InnerHeight`를 기초 상부면~상부슬래브 하부면의 내부 유효높이로 통일. 전체 높이(`RoomHeight`)에는 상부슬래브 두께를 추가해 상부 레벨·상부슬래브 상단을 올리고, 외벽 높이는 입력 내부 높이를 그대로 유지한다.
- [x] DTO XML 문서와 UI 레이블을 "내부 높이(슬래브 하부)"로 갱신. 이토밸브실의 중간슬래브 경로는 기존처럼 1F/2F 내부 높이와 슬래브 두께를 합산한다.
- [x] 이토·제수·공기밸브실 3종에 대해 상부 레벨/상부슬래브 상단과 외벽 높이를 검증하는 회귀 테스트를 추가.
- [x] 검증: Application 빌드 오류 0·경고 0, UI 빌드 오류 0(기존 Revit 참조 경고 4개). 테스트 프로젝트는 기존 `DHBIMWATER.Core.pdb` 잠금 후 PDB 제외 재실행 시 120초 내 완료하지 않아 결과 미확인.

### 공기밸브실 내부 높이 기준 검토 (2026-07-30)
- [x] 공기밸브실도 제수밸브실과 동일한 공통 `InnerHeight` 산정 경로를 사용한다. 이토밸브실의 중간슬래브 경우만 별도 분기하므로, 공기밸브실 입력값 역시 현재 기초 상부~상부슬래브 상부 전체 높이로 적용된다.
- [ ] TODO: 내부 유효높이(기초 상부~상부슬래브 하부)를 입력 기준으로 확정하면 제수·공기밸브실을 함께 변경하고, 이토밸브실(중간슬래브 없음)도 동일 기준 적용 여부를 별도 결정.

### 제수밸브실 내부 높이 기준 검토 (2026-07-30)
- [x] 제수밸브실의 `InnerHeight`는 현재 기초 상부~상부슬래브 **상부**까지의 전체 높이로 적용됨을 확인. `RoomHeight()`가 입력값을 그대로 반환하고, `TopElevation()`과 상부슬래브 `ElevationZ`가 그 값을 사용한다.
- [x] 따라서 상부슬래브 하부(실제 내부 천장면)는 `InnerHeight - UpperSlabThickness`이며, 외벽도 같은 값으로 생성된다. UI 표기는 "내부 높이"이나 구현 기준과 불일치한다.
- [ ] TODO: 사용자가 원하는 기준(기초 상부~상부슬래브 하부)으로 변경할 경우 `RoomHeight()`에 상부슬래브 두께를 더하고, 회귀 테스트·UI 안내 문구를 함께 갱신.

### 수량산출 DHBIMWATER 색상 Converter 재빌드·재배포 (2026-07-29)
- [x] 수량산출 리본은 DHBIMWATER UI가 아니라 Costura로 내장된 `DHBoost.Revit`의 `QuantityCommandForWater`를 실행함을 확인. 색상 Converter/`TitleBar` 구현의 실제 소유처는 `DHBoost_24`이다.
- [x] DHBoost 원본에는 `BrandProfile.DHBIMWATER`에서 `TitleBar`의 Primary 브러시를 `#3498DB`/`#2980B9`/`#EAF4FC`로 바꾸는 구현과 Water 전용 커맨드가 존재한다.
- [x] 그러나 현재 참조 라이브러리(`src/DHBIMWATER.Revit/libs/DHBoost/DHBoost.Revit.dll`), Release 출력 DLL, Revit 2026 Addins 배포 DLL의 SHA-256이 모두 서로 다름. 최신 DHBoost 변경이 DHBIMWATER 패키지에 다시 내장·빌드·배포되지 않아 이전(보라색) 번들이 실행되는 것이 미반영 원인이다.
- [x] DHBoost Revit 2026 Release를 PDB 생성 없이 재빌드(오류 0개)하고 새 `obfuscated/DHBoost.Revit.dll`로 `libs/DHBoost/DHBoost.Revit.dll`을 교체했다.
- [x] `dotnet build src\DHBIMWATER.Revit\DHBIMWATER.Revit.csproj -c Release --no-restore -p:DebugType=None` 오류 0개(기존 경고 233개)로 Costura 내장 및 Revit 2026 Addins 배포를 갱신했다.
- [x] 검증: 새 DHBoost 출력 DLL과 DHBIMWATER 참조 DLL SHA-256 일치, DHBIMWATER Release 출력 DLL과 `%APPDATA%\\Autodesk\\Revit\\Addins\\2026\\DHBIMWATER\\DHBIMWATER.Revit.dll` SHA-256 일치.
- [ ] TODO: Revit을 완전히 재시작한 후 수량산출/설정 창의 파란색 강조 버튼·탭·체크박스를 실제 화면에서 확인.

### 내장 DHBoost 명령 생성 실패 수정 — 중첩 Costura 모듈 초기화 (2026-07-29)
- [x] 증상: 수량산출/수량산출설정 버튼 클릭 시 "내장된 DHBoost 명령을 만들 수 없습니다: ...QuantityCommandForWater / ...QuantitySettingCommandForWater".
- [x] 원인 규명: 배포본 DLL로 재현. `Assembly.Load("DHBoost.Revit")`는 성공하나, DHBoost 자체 Costura 리졸버가 `<Module>` 정적 생성자에서 등록되는데 바이트 배열로 로드된 어셈블리는 해당 생성자가 자동 실행되지 않는다. 그 결과 기반 타입 해석 시 `DHBoost.UI`/`Application`/`Infrastructure` 로드가 모두 `FileNotFoundException`으로 실패하고, `Assembly.CreateInstance(string)`이 이를 삼켜 null을 반환했다.
- [x] 수정: `EmbeddedDhBoostCommand.Execute`에서 `Assembly.Load` 직후 `RuntimeHelpers.RunModuleConstructor(assembly.ManifestModule.ModuleHandle)`로 모듈 초기화를 강제. 이어서 `CreateInstance(string)` 대신 `GetType(CommandTypeName, throwOnError: true)` + `Activator.CreateInstance`로 전환해 실패 원인이 대화상자에 그대로 노출되게 했다. (`src/DHBIMWATER.Revit/Commands/QuantityCommand.cs`)
- [x] `dotnet build src\DHBIMWATER.Revit\DHBIMWATER.Revit.csproj -c Release --no-restore` 오류 0개(경고 8개, 모두 기존 경고).
- [x] 재배포된 Release DLL로 재검증: 모듈 초기화 강제 후 `DHBoost.UI/Application/Infrastructure/Domain/Shared`, `ClosedXML`, `MahApps.Metro.IconPacks` 전부 해석 성공.
- [x] Revit 2026 실사용 검증 완료(사용자 확인, 2026-07-29): 수량산출 창 정상 표시. `DHBoost.Revit.dll` 별도 배포 없이 `DHBIMWATER.Revit.dll` 단독으로 수량산출 동작 확인.

### DHBoost.Revit.dll 미배포 시 수량산출 동작 정적 검증 (2026-07-29)
- [x] Release `DHBIMWATER.Revit.dll`(66.6MB) 매니페스트 리소스 77개 확인. `costura.dhboost.revit.dll.compressed` 포함.
- [x] `DHBoost.Revit.dll` 자체가 Costura 패킹본임을 확인(리소스 76개). `DHBoost.Application/Domain/Infrastructure/Shared/UI` 5개가 그 안에 내장되어 있어 2단 중첩 해석으로 전체 트리가 해결된다.
- [x] DHBoost 측 Costura 로더(`A.b`)가 `<Module>` cctor에서 `AssemblyLoadContext.Default.Resolving`에 등록되고, 리소스를 `Assembly.GetExecutingAssembly()`에서 읽음을 IL 디컴파일로 확인. 바이트 배열 로드된 어셈블리에서도 정상 동작한다.
- [x] `DHBoost.Revit.Commands.CommandBase.Execute`가 `ServiceContainer.A(commandData.Application)`로 DI 컨테이너를 자체 생성함을 확인. `DHBoost.Revit.App.OnStartup`이 실행되지 않아도 무방하다.
- [x] 양쪽 번들에 중복 내장된 3rd-party 60개(ClosedXML, MahApps.IconPacks, ExcelDataReader, M.E.DependencyInjection 등) 버전이 전부 일치. 이름 기준 해석 충돌 위험 없음.
- [x] `DHBoost.Application/Domain/Infrastructure/Shared/UI`에서 `Assembly.Location`·`BaseDirectory`·`LoadFrom` 사용 없음. `Assembly.Location`을 쓰는 곳은 DHBoost 자체 `App`/`RibbonBuilder`뿐이며 DHBIMWATER 경로에서는 실행되지 않는다.
- [x] 결론: `DHBoost.Revit.dll` 별도 배포 불필요. `.addin` + `DHBIMWATER.Revit.dll` + `DidasUsage.dll`/`DidasReader.dll`만 배포하면 된다.
- [x] 잔여 리스크였던 `pack://application:,,,/DHBoost.UI;component/Resources/Icons/` pack URI 해석: Revit 2026 실행으로 확인 완료(2026-07-29). 바이트 배열 로드된 `DHBoost.UI`에서도 pack URI 정상 해석됨.

### DHBoost.Combined 제거 및 DHBoost.Revit 내장 배포 전환 (2026-07-29)
- [x] 전환 경로 확인: `DHBoost.Combined.dll` 참조를 제거하고, `DHBoost.Revit.dll`을 Costura로 내장한다. DHBIMWATER 프록시 Revit 명령을 통해 기존 수량산출 리본 기능을 유지한다.
- [x] DHBoost의 RevitAPI/RevitAPIUI 참조를 Revit 2026으로 전환해 Release DLL을 생성하고, `libs/DHBoost/DHBoost.Revit.dll`로 교체했다.
- [x] `DHBoost.Combined.dll` 참조와 파일을 제거. 수량산출 리본은 DHBIMWATER 프록시 명령 2개를 실행하고, 프록시는 Costura에 내장된 `DHBoost.Revit`의 `*ForWater` 명령으로 위임한다.
- [x] `dotnet build src\DHBIMWATER.Revit\DHBIMWATER.Revit.csproj -c Release --no-restore` 오류 0개. `costura.dhboost.revit.dll.compressed` 포함 및 `costura.dhboost.combined.dll.compressed` 부재를 확인했다.
- [ ] TODO: Revit을 종료 후 새 Release `DHBIMWATER.Revit.dll`을 배포하고, DHBoost 폴더 없이 수량산출/설정 버튼을 실제 실행해 확인.

### DHBoost 단일 파일 내장 배포 설계 검토 (2026-07-29)
- [x] Costura로 `DHBoost.Revit.dll` 및 그 비-Revit 의존성을 `DHBIMWATER.Revit.dll`에 내장하는 것은 가능. RevitAPI/RevitAPIUI는 호스트가 제공하므로 내장하지 않는다.
- [x] 현 소스에서 `DHBoost.Combined.dll`은 csproj 참조·Costura 내장만 되고 DHBIMWATER 코드가 타입을 직접 사용하지 않는다. `DHBoost.Revit.dll`은 수량산출 리본이 파일 경로로 직접 실행하는 별도 커맨드 어셈블리이므로, 두 파일은 역할이 다르다.
- [x] Revit은 내장 리소스의 `IExternalCommand`를 직접 리본 대상으로 찾을 수 없으므로, 본 애드인 DLL에 수량산출/설정 프록시 명령 2개를 두고 이 명령이 내장된 DHBoost 명령을 위임 실행해야 한다.
- [x] 전제: 내장할 DHBoost.Revit은 실행 대상 Revit(현재 2026)의 API 버전으로 빌드해야 하며, `.addin` 매니페스트는 DHBIMWATER 진입점용으로 계속 필요하다. 현재 DidasUsage/DidasReader도 별도 배포 중이므로 완전 단일 DLL을 목표로 하면 함께 내장 여부를 별도 점검해야 한다.
- [x] 사용자 결정: `DidasUsage.dll`과 `DidasReader.dll`은 별도 배포를 유지한다. 단일 파일 내장 범위는 DHBoost 계열만으로 한정한다.

### 수량산출 리본 명령 초기화 오류 점검 (2026-07-29)
- [x] `QuantityRibbonModule`이 지정한 `QuantityCommandForWater`/`QuantitySettingCommandForWater` 공개 타입은 Revit 2026 배포 DLL에 실제 존재하며, 공통 `CommandBase`를 통해 `IExternalCommand`를 구현함을 확인.
- [x] `DHBoost.Revit.dll`은 지정 경로(`<Addins>\\2026\\DHBoost\\DHBoost.Revit.dll`)에 존재하므로 DLL 파일 경로를 찾지 못한 오류는 아님.
- [x] `DHBoost.Revit.addin`은 현재 없음. DHBIMWATER 버튼은 `PushButtonData`에 DLL 경로와 클래스명을 직접 전달하므로 매니페스트 부재가 해당 클래스 탐색의 필수 조건은 아님. 단, DHBoost `App`의 시작 초기화(AssemblyResolve/ExternalEvent)는 실행되지 않음.
- [x] 배포 DLL 메타데이터가 `RevitAPI`/`RevitAPIUI` 25.4.10.0을 참조하는 반면 설치 Revit 2026 API는 26.4.0.0임을 확인. 2026 호환 DHBoost 빌드·배포 여부를 결정해야 함.

### DHBoost 단일 DLL 배포 가능성 검토 (2026-07-28)
- [x] `DHBoost.Combined.dll`을 일반 .NET 참조로 두고 Costura.Fody가 `DHBIMWATER.Revit.dll`에 임베딩하는 구조를 확인. 기존 Release DLL에 `costura.dhboost.combined.dll.compressed` 리소스가 실제 포함되어 있다.
- [x] RevitAPI/RevitAPIUI는 Revit 프로세스가 제공하는 호스트 어셈블리이므로 `<Private>False</Private>`를 유지하고 임베딩하지 않는다. DHBoost는 독립 배포 대상이며 임베딩 가능하다.
- [ ] TODO: DHBoost 소스 DLL을 직접 참조하도록 전환할 경우, DHBoost 내부 의존 DLL도 함께 Costura에 포함하거나 현재의 `DHBoost.Combined.dll` 단일 병합본을 유지해야 한다.

### 제수밸브실 보·상부슬래브 BoundingBox 결합 전환 (2026-07-28)
- [x] 상부슬래브 식별 메타데이터가 없는 실제 Revit 문서에서도 결합되도록, 제수밸브실 보와 Floor의 3차원 BoundingBox 겹침으로 Join 대상을 판별하도록 전환. `Document.Regenerate()` 후 Min/Max의 X/Y/Z 범위가 모두 겹치는 Floor만 대상이다.
- [x] `dotnet build src\DHBIMWATER.Infrastructure\DHBIMWATER.Infrastructure.csproj -c Release --no-restore` 오류 0개(기존 경고 149개).
- [ ] TODO: Revit에서 제수밸브실을 새로 생성해 보-슬래브 Join 결과를 확인.

### 제수밸브실 보·상부슬래브 미결합 검토 (2026-07-28)
- [x] 실제 Revit 확인 결과를 기준으로 생성 경로를 점검. 슬래브가 먼저 생성되는 순서는 정상이나, `RevitBeamCommandRepo`가 `ElementIntersectsElementFilter`로 후보를 먼저 제거하므로 배치 오차 또는 접촉 판정 시 `JoinGeometryUtils.JoinGeometry` 호출 자체가 생략된다.
- [x] 수정: 제수밸브실 보(`VR-B`)만 동일 Zone의 `DH_Part = "상부슬래브"`를 직접 찾아 Join. 기존 결합 여부는 건너뛰고, 대상 없음·Join 실패 시 보/슬래브 ID를 포함해 오류를 표시.
- [x] `dotnet build src\DHBIMWATER.Infrastructure\DHBIMWATER.Infrastructure.csproj -c Release --no-restore` 오류 0개. Debug 빌드는 기존 프로세스의 `DHBIMWATER.Application.pdb` 잠금으로 실행 불가.
- [ ] TODO: Revit에서 제수밸브실을 새로 생성해 보-상부슬래브 결합 결과를 육안 확인.

### 밸브실 중간벽 안목거리 및 제수밸브실 기둥 상단 오프셋 (2026-07-28)
- [x] 이토밸브실 중간벽 위치를 좌측 외벽 내측면(X=0) → 중간벽 좌측면 안목거리로 변경. 생성 중심선은 `안목거리 + 중간벽 두께/2`로 환산.
- [x] UI 문구와 `MudValveRoomSpecDto` 주석을 동일한 안목거리 기준으로 갱신.
- [x] 제수밸브실 기둥 상단을 `밸브실 상부슬래브` 레벨에 구속하고, `TopOffset = -UpperSlabThickness`로 슬래브 하부에 맞춤.
- [x] 계산기 회귀 테스트 2건 추가(중간벽 좌측면 기준, 기둥 상단 오프셋). Application·UI 빌드 오류 0개.
- [ ] 기존 UI 테스트 프로젝트는 제거된 수량산출 타입을 참조하는 기존 파일의 컴파일 오류로 전체 테스트 실행 불가(새 테스트와 무관).

### 펌프장 상부슬래브 레벨 누락 오류 점검 (2026-07-28)
- [x] 원인 확인: `CreatePumpingStationUseCase`가 기존 레벨을 `Contains(lvl.Name)`으로 부분 비교함. 기존 `수조부 상부슬래브`/`밸브실 상부슬래브`가 있으면 펌프장의 정확한 `상부슬래브` 레벨을 생성하지 않고 그 레벨을 갱신함.
- [x] 이후 `RevitSlabCommandRepo`는 `Equals(slabDef.LevelName)`으로 정확히 `상부슬래브`를 찾아 실패. 코드 변경 없이 점검 완료.
- [x] 수정: 기존 레벨 판별을 대소문자 무시 정확 비교(`Equals(..., OrdinalIgnoreCase)`)로 변경. `상부슬래브`가 없으면 정상 생성됨.
- [x] 검증: `dotnet build src\DHBIMWATER.Application\DHBIMWATER.Application.csproj --no-restore -p:DebugType=none` 오류 0개(기존 nullable/Visual Studio PDB 잠금 경고 35개).
- [x] 영향 점검: 변경 범위는 펌프장 레벨 생성 루프만이며, 다른 생성 경로에는 적용되지 않음. 기존 부분 일치에 의존한 별도 호출부는 없음.
- [x] 범위 확정: 레벨명은 기존 범용명(`상부슬래브` 등)을 유지하고, 이름 변경·시트/태그 연동 수정은 하지 않음.
- [x] 전체 솔루션 빌드 검증: Application까지 정상 빌드했으나 UI 임시 WPF 프로젝트에서 기존 `TitleBar.xaml.cs(27)`의 `InitializeComponent` 미정의(CS0103)로 솔루션 빌드 중단. 레벨 변경과 무관한 오류이며, Application 단독 빌드는 오류 0개.

### 이토밸브실 중간벽 Y방향 단일화 + 좌측벽 offset 입력 (2026-07-28)
- [x] 중간벽을 **1개**로 단순화. `MudValveRoomSpecDto`의 `IntermediateWallCount`(int) → `IntermediateWallOffset`(double, 좌측 외벽 내측 X=0 → 중간벽 중심 거리 mm) 교체.
- [x] `CalculateWalls`: 중간벽을 offset 위치(X=offset)에 **폭(Y) 방향**(0→InnerWidth)으로 뻗는 단일 벽으로 생성. 중간슬래브 시 1F/2F 2단 분리 로직 유지.
- [x] `ValveRoomViewModel`: `IntermediateWallCount` → `IntermediateWallOffset`(기본 InnerLength/2=2000). Summary "중간벽 N개"→"중간벽". 프리셋/DTO 생성 갱신.
- [x] `ValveRoomView.xaml`: "중간벽 개수(개)" → "좌측벽→중간벽 거리(mm)". 가시성은 기존 `IntermediateWallVisibility`(IsMud && HasIntermediateWall) 유지 → 체크박스 true일 때만 표시.
- [x] 전체 솔루션 `dotnet build` 오류 0개.

### 밸브실 축 스왑(X=길이, Y=폭) · 보-슬래브 결합 · 버림 오프셋 (2026-07-28)
- [x] `ValveRoomGeometryCalculator` 전치(A안): X축=길이(`InnerLength`), Y축=폭(`InnerWidth`).
  - `CalculateSlabs`: center·extent를 X=길이/Y=폭으로 교체. `CalculateWalls`: 외벽 X extent=`InnerLength`, Y extent=`InnerWidth`. 중간벽은 폭(Y)을 등분하고 길이(X)로 뻗도록 전치. `CalculateBeams`: X방향 보 span=`InnerLength`, Y방향 보 span=`InnerWidth`. 기둥은 offset/spacing 기반이라 변경 불필요.
- [x] 제수밸브실 보-슬래브 결합: `RevitBeamCommandRepo`에서 **HAUNCH와 밸브실 보(`ElementCode=="VR-B"`)만** `beamDef.ZJustification`(밸브실=원점2) 적용. **PumpingStation GIRDER·Reservoir 보는 원본대로 Revit 기본 Z맞춤 유지**(모든 보 일괄 적용은 회귀라 롤백).
- [x] 밸브실 보 Z 배치: 원점(2) 기준 + `Z_OFFSET_VALUE = -(보높이/2)`로 보를 아래로 내려 보 상단을 보 z(=상부슬래브 상단)에 맞춤. 보 높이는 타입 파라미터(`STRUCTURAL_SECTION_COMMON_HEIGHT`→`"h"`/`"H"`)에서 읽음(`GetBeamHeightFt`). `JoinWithUpperSlab` 전에 적용해 절단 성립. (가정: 선택 보 패밀리가 단면 높이 파라미터 노출 — Revit 육안 확인 필요)
- [x] 버림콘크리트를 기초보다 사방 `PlainConcreteThickness`만큼 확장(`foundationX/Y + PlainConcreteThickness*2`).
- [x] `dotnet build` Application·Infrastructure 오류 0개. (기존 nullable/PDB 잠금/RevitAPI 아키텍처 경고만 유지)
- TODO(사용자): Revit에서 제수밸브실 생성으로 보-슬래브 절단 결과 및 축 방향 육안 확인 필요.

## 완료된 작업(이전)

### 이토밸브실 중간슬래브 층고 정합 및 중간슬래브 레벨 추가 (2026-07-28)
- [x] `RoomHeight`(중간슬래브 케이스)에 `+ UpperSlabThickness` 추가 → 상부슬래브 상단 EL = `기초상단 + 1F안목 + 중간슬래브두께 + 2F안목 + 상부슬래브두께`. "기초 상부면~상부슬래브 상부면" 정의 정합.
- [x] 중간슬래브 `ElevationZ`를 `baseZ + Floor1`에서 `baseZ + Floor1 + IntermediateSlabThickness`로 보정(슬래브 상단면 기준, 1F 외벽 상단과 겹침 제거).
- [x] 2F 외벽·중간벽 높이의 `- UpperSlabThickness` 제거 → 2F 안목높이 그대로 적용. 상단은 상부슬래브 하단과 맞물림.
- [x] `IntermediateLevelName = "밸브실 중간슬래브"` 상수 추가, `CalculateLevels`에서 이토밸브실+중간슬래브일 때 중간슬래브 상단(2F 바닥) 참고 레벨 생성. 벽·슬래브는 기존대로 미연결 높이/기초상부 레벨 호스팅 유지(중간슬래브 레벨 구속 없음).
- [x] `dotnet build src\DHBIMWATER.Application` 오류 0개 확인(기존 nullable 경고만 유지).

### 밸브실 외벽 코너 겹침 제거 (ㅁ자 정합) (2026-07-28)
- [x] `ValveRoomGeometryCalculator.CalculateWalls`에서 외벽 4면의 코너 겹침 제거. 중심선 위치는 유지하되 끝점만 조정: 가로 벽체는 양끝을 `t/2` 바깥으로, 세로 벽체는 양끝을 `t/2` 안쪽으로(`t = OuterWallThickness`).
- [x] 외벽 외면이 정확한 ㅁ자를 구성하도록 정합. 이토밸브실 2단 외벽도 동일 `outerCorners`를 공유해 자동 적용. **중간벽은 기존대로 유지.**
- [x] 빌드 검증 완료 — 이후 작업(「이토밸브실 중간슬래브 층고 정합…」)에서 `dotnet build src\DHBIMWATER.Application` 오류 0개 확인. 당시 `Core.pdb` 잠금(CS2012)은 Revit 실행으로 인한 일시적 현상이었음.

### 중간 슬래브 기준 중간벽 분리 (2026-07-28)
- [x] 이토밸브실에서 중간 슬래브와 중간벽을 함께 선택하면, 각 중간벽을 하부 벽체(`Floor1InnerHeight`)와 상부 벽체(`Floor2InnerHeight - UpperSlabThickness`)로 분리 생성.
- [x] 상부 중간벽의 기준 오프셋을 `Floor1InnerHeight + IntermediateSlabThickness`로 지정해 중간 슬래브 상단에서 시작하도록 처리.
- [x] `dotnet build src\\DHBIMWATER.Application\\DHBIMWATER.Application.csproj --no-restore -p:DebugType=none` 오류 0개 확인(기존 nullable/PDB 잠금 경고 유지).

### 상부 슬래브 접합 및 벽체 높이 보정 정리 (2026-07-28)
- [x] 밸브실 벽체 중 상부 슬래브에 닿는 외벽·중간벽의 높이에서 `UpperSlabThickness`를 차감. 중간 슬래브 하부 외벽은 기존 높이를 유지.
- [x] `RevitBeamCommandRepo`가 교차하는 Floor 중 `DH_Part = "상부슬래브"`인 요소만 찾아 결합하도록 한정.
- [x] `dotnet build`로 Application·Infrastructure 프로젝트 오류 0개 확인. 기존 Revit 참조 버전/아키텍처 및 Visual Studio PDB 잠금 경고는 유지.

### 펌프장 벽체 높이·보/상부슬래브 결합 구현 점검 (2026-07-28)
- [x] `PumpingStationViewModel`의 `H5 = H2 + H3 + H4 - T1` 산정과 `PumpingStationGeometryCalculator`의 벽체·보 정의를 대조 점검.
- [x] `RevitWallCommandRepo`는 전달받은 높이를 그대로 생성하고, `RevitBeamCommandRepo`는 생성 후 교차하는 Floor와 결합하는 동작을 확인.
- 코드 변경 없음. 세부 판정은 작업 응답에 기록.

### 선형 패밀리 배치 ↔ 관로 DirectShape 좌표 정합 구현 (2026-07-28)
- [x] `AlignmentReferencePoint` 공용 헬퍼를 추가하고 Piping/Alignment 경로가 첫 유효 정점의 정수 미터 반올림(`AwayFromZero`) 기준점을 공유하도록 통일.
- [x] `AlignmentFamilyPlacementRequest`/`AlignmentPlacementOrigin` 및 Beam·Pipe 배치 인터페이스를 확장해 기준점, `ZDatum`, 선택적 공유좌표 기록을 명시적으로 전달.
- [x] Beam·Pipe Revit 리포지토리의 좌표 변환을 DirectShape와 동일한 식으로 정합하고 `Invert`/`Crown`의 관경 절반 Z 보정을 적용. 기존 `AlignTangent`, 짧은 세그먼트 스킵, 파이프 연결은 유지.
- [x] UseCase가 트랜잭션 안에서 필요 시 공유좌표를 기록하고, 기준점이 미입력(모두 0)이면 공용 헬퍼로 계산하도록 구현. Alignment UI는 기존대로 안전한 기본값(`AsIs`/`false`)을 사용.
- [x] 반올림 경계값(`±.5`) 단위 테스트 추가. `dotnet build DHBIMWATER.sln --no-restore -p:DebugType=none` 오류 0개(기존 Revit/Visual Studio 파일 잠금 및 참조 경고 247개). 기존 UI 테스트 프로젝트는 삭제된 수량산출 타입 참조로 컴파일 불가하여, GIS 테스트 파일만 링크한 임시 프로젝트에서 7건 통과.

### 선형 패밀리 배치 ↔ 관로 DirectShape 좌표 불일치 분석 + 지시서 작성 (2026-07-28)
- [x] 같은 SHP로 `AlignmentFamilyPlacementViewModel`(StructuralFraming/Pipe)과 `PipingViewModel`(DirectShape) 배치 위치가 어긋나는 원인 규명.
  - 근본 원인: 두 경로가 첫 정점을 기준점으로 빼서 원점 근처로 옮기지만, Piping은 `Math.Round(AwayFromZero)`로 **정수 미터 반올림한** 기준점(`PipingViewModel.cs:78-80`)을, Alignment 리포지토리는 **반올림 안 한 원본 첫 정점**(`RevitAlignmentBeamPlacementRepo.cs:19`, `RevitAlignmentPipePlacementRepo.cs:19`)을 사용 → 첫 정점 좌표 소수부만큼(축당 최대 ±0.5m, 대각선 ~0.7m) 어긋남.
  - 파생: Alignment 경로는 `ZDatum`(Invert/Crown) 미반영, 공유좌표(`SetInternalOriginSharedPosition`) 미기록, request에 기준점 필드 자체가 없음.
  - 형상 자체는 동일(`SampleSegments`가 절점을 경계에 포함) — 위치 오프셋만 문제.
  - 정확도: 상대 형상은 양쪽 동일, 절대(측량) 위치는 공유좌표를 기록하는 **Piping이 기준(정답)**.
  - `ZSource`는 현재 Piping 경로에서도 미사용 값임을 확인 → 정합 범위에서 제외.
- [x] 완전 정합(B안) 지시서 작성 — `docs/19_선형패밀리배치_좌표정합_지시서.md` (기준점 반올림 통일 + ZDatum 반영 + 공유좌표 기록, 변경 파일 8개 + 검증 시나리오 5건).
- [x] 지시서에 따른 실제 코드 수정 및 빌드/테스트 검증 완료 — 위 「선형 패밀리 배치 ↔ 관로 DirectShape 좌표 정합 구현 (2026-07-28)」 항목 참조.

### 관로 선형 UI — 관종 입력 헤더 표시 (2026-07-28)
- [x] `PipingView`의 파일별 관종 입력 칸에 `파일명 · 관종 · 읽기 결과` 열 헤더를 추가해 입력 목적을 명확히 표시.
- 관종 입력값은 생성 관로 DirectShape의 이름(`관로_{관종}_{레코드번호}`)과 `DH_Part` 파라미터에 저장됨을 확인.
- 검증: `dotnet build src\\DHBIMWATER.UI\\DHBIMWATER.UI.csproj --no-restore -p:DebugType=none` 오류 0개(기존 경고 239개 및 Visual Studio PDB 잠금 경고 포함).

### 관로 선형 배치 — 절점 분절 누락 수정 (2026-07-23)
- [x] `SampleSegments`의 분절 경계에 interval 배수와 폴리라인 절점 누적거리의 합집합을 적용하고, tolerance 기준으로 중복을 제거해 코너를 가로지르는 직선 현 및 길이 0 세그먼트를 방지.
- [x] `SamplePoints`를 인덱스 곱 방식으로 전환해 부동소수 누산 오차를 제거하고, interval 배수만 반환하는 기존 계약을 유지.
- [x] `RevitAlignmentPipePlacementRepo`에 `ShortCurveTolerance` 가드를 추가하고, 짧은 구간을 건너뛴 뒤에도 이전 파이프와 다음 파이프를 연결하도록 유지.
- [x] 회귀 테스트 3건 추가 — 비정렬 절점 분절, interval과 절점 중복 시 0길이 방지, 전체 길이 연속성.
- 검증: `dotnet build DHBIMWATER.sln --no-restore -p:DebugType=none` 오류 0개(기존 경고 151개 및 실행 중인 Revit/Visual Studio의 Addins DLL 잠금 경고 포함). 기존 수량산출 타입을 참조하는 별도 테스트 잔재가 원본 테스트 프로젝트 컴파일을 막으므로 해당 파일을 수정하지 않고, GIS 테스트 파일만 링크한 임시 프로젝트에서 `dotnet test ... -p:DebugType=none`으로 기존 3건 + 신규 3건, 총 6건 통과.

### 관로 선형 배치 — 절점 분절 누락 결함 분석 + 지시서 작성 (2026-07-23)
- [x] `AlignmentFamilyPlacementView`로 배치한 부재가 도면 절점에서 끊기지 않는 원인 규명.
  - 근본 원인: `AlignmentIntervalSampler.SampleSegments`(`Core/Gis/AlignmentIntervalSampler.cs:32`)가 분절 경계를 `interval` 배수 + 최종 끝점으로만 잡고, 폴리라인 정점의 누적 호길이를 경계에 포함하지 않음 → 절점을 가로지르는 구간이 직선 현(chord)이 되어 코너가 소실됨.
  - 파생 이슈 3건: `SamplePoints`의 부동소수 누산 오차, 절점 경계 추가 시 늘어나는 짧은 구간에 대한 파이프 리포지토리의 가드 부재, 절점/간격 배수 중복으로 인한 길이 0 세그먼트 가능성.
  - 영향 범위: 보(`RevitAlignmentBeamPlacementRepo:26`), 파이프(`RevitAlignmentPipePlacementRepo:25`) 양쪽 동일.
- [x] 수정 지시서 작성 — `docs/18_관로선형_절점분절_지시서.md` (수정 4건 + 회귀 테스트 3건 + 완료 조건).
- [x] 지시서에 따른 실제 코드 수정 및 빌드/테스트 검증 완료 — 위 「관로 선형 배치 — 절점 분절 누락 수정 (2026-07-23)」 항목 참조.

### 관로 UI 공통 스타일 적용 (2026-07-23)
- [x] `PipingView`, `AlignmentFamilyPlacementView`, `PipeLayoutView`의 기본 렌더링 컨트롤에 기존 공통 스타일을 연결.
  - 버튼(주요/보조), 텍스트박스, 체크박스, 콤보박스, 탭, 목록, 그룹박스에 `Generic.xaml`의 기존 스타일 키를 적용.
  - 도면 Canvas의 색상·상호작용 및 ViewModel 바인딩은 변경하지 않음.
- 검증: `dotnet build src\\DHBIMWATER.UI\\DHBIMWATER.UI.csproj --no-restore -p:DebugType=none` 오류 0개(기존 경고 239개, Visual Studio PDB 잠금 경고 포함).

### 수량산출 — DHBoost 연동 전환 (2026-07-22)
- [x] **DHBIMWATER 수량산출 코드 전량 제거 (파일 90개 삭제)**
  - Core `Quantity/` 전체 + `Settings/` 중 수량 전용 7개(`QuantitySettings`, `ProjectSettings`, `Formwork`, `Deduction`, `LossRate`, `RebarRatio`, `Rebar`)
  - Application: `Interfaces/{Quantity,Storage,Settings,Geometry}`, `UseCases/QuantityCalculator/`, `QuantityRuleEngine`·`LossRateCalculator`·`RebarApproximationCalculator`
  - Infrastructure: `Repositories/Revit/Quantity/`(Extractor 15개), `Mock/Quantity/`, `Revit/Storage/`(저장소 4개), `Revit/Geometry/`, `FileSystem/DhcfgRepo`, `Storage/Schemas/`(4개), `Helpers/QuantityExtractorHelper`, `Services/Excel/ClosedXmlExcelWriter`
  - UI: `ViewModels/Quantity/`, `Views/Quantity/`, `Converters/{QuantityStatus,GroupElementCount}Converter`
  - Revit: `Commands/Quantity/`
  - DI 등록 정리 4곳(Application/Infrastructure/UI/Revit) + Sandbox 수량산출 진입 버튼 제거
  - **삭제 안전성 검증**: 삭제 전 수량산출 외부의 역참조를 전수 조사하고, 애매한 타입(`IFaceClassifier`, `IIntersectingElementFinder`, `ProjectSettings` 계열, `IExcelExporter`, `RebarSettings`)은 개별 추적해 수량산출 전용임을 확인. 삭제 후 대상 디렉터리 6곳이 모두 비워진 것으로 교차 확인. 엑셀 **읽기**(`ExcelReader`, 모델링에서 사용)는 `Repositories/DB`에 있어 영향 없음
- [x] **리본 버튼을 DHBoost 커맨드로 배선** — `QuantityRibbonModule`이 컴파일 타임 참조 없이 `PushButtonData`의 어셈블리 경로 + 클래스 풀네임 문자열로 실행. 경로는 자기 어셈블리 위치 기준 형제 폴더(`<Addins>\DHBoost\DHBoost.Revit.dll`)로 해석해 Revit 버전 하드코딩 없음. DHBoost 미설치 시 패널 자체를 만들지 않고 로그만 남김
  - 참조를 걸지 않는 이유: `libs/DHBoost/DHBoost.Combined.dll`(ILRepack 병합본)과 `DHBoost.Revit.dll`(Costura 임베드)에 `DHBoost.Infrastructure` 타입이 중복 존재 → 타입 충돌
- [x] **호스트별 타이틀바 브랜딩** (DHBoost 리포 측 작업) — `TitleBar`에 `Brand` DependencyProperty(기본값 `DHBoost`) 추가, WATER 로고(`PackIconMaterial Water`/`#3498DB`)를 두 번째 아이콘으로 두고 토글. `QuantityCommandForWater`/`QuantitySettingCommandForWater` 파생 커맨드가 제목·브랜드를 지정. 수동입력 창은 부모 창의 브랜드를 상속
  - DHBIMWATER의 `TitleBar`는 **무변경** — 열리는 창이 DHBoost의 `QuantityView`이므로 DHBoost 쪽 `TitleBar`만 관여함
  - DHBoost의 나머지 7개 창(LandXml/Mass/Excel/GuideLine/Modeling)은 `Brand` 미지정 → 기본값으로 기존과 동일
- 빌드 검증: DHBIMWATER 오류 0개(경고 88개) / DHBoost 오류 0개(경고 44개)
- [ ] **Revit 실측 필요** — DHBIMWATER 탭에서 버튼 2개가 뜨는지, 창 제목이 `DHBIMWATER - 수량산출`이고 로고가 파란 물방울인지, DHBoost 탭에서는 `DHBoost - 수량산출` + 보라 번개인지 확인
- 참고: `DHBIMWATER.UI/Styles/DataGrids.xaml`의 `QuantityDataGridStyle`과 `Resources/Quantity/*.png`는 이제 미사용이나 기존 코드라 삭제하지 않음. `DocumentationRibbonModule`의 주석 처리된 `QuantityCommand2~4` 플레이스홀더도 동일
...
### GIS 선형 배치 (Alignment)
- [x] **선형 따라 보(StructuralFraming) 배치를 점 배치 → 선 기반 보 생성으로 수정** (2026-07-22)
  - 증상: `RevitAlignmentBeamPlacementRepo.PlaceAlong`이 `SamplePoints`로 6m마다 **점**만 뽑아 `NewFamilyInstance(point, …, NonStructural)`로 점 기반 패밀리를 꽂고 접선 회전만 줌 → 동일 길이 패밀리가 점마다 배치된 것처럼 보임
  - 수정: `AlignmentIntervalSampler.SampleSegments`(이미 존재, 파이프 배치와 동일)로 6m마다 끊어 시작/끝점 쌍을 얻고 `NewFamilyInstance(Line.CreateBound(start, end), …, StructuralType.Beam)`로 선 기반 보 생성. 접선 회전 제거(커브가 곧 방향)
  - `doc.Application.ShortCurveTolerance`보다 짧은 꼬리 세그먼트는 건너뛰어 예외 방지
  - `MockAlignmentBeamPlacementRepo` 카운트도 `SampleSegments` 기준으로 통일(파이프 Mock과 동일)
  - `IAlignmentBeamPlacementRepo.alignTangent`는 선 기반에선 무의미해졌으나 시그니처 변경 파급(UseCase·ViewModel·Mock) 방지 위해 유지(주석만 명시). 빌드 검증: Infrastructure 오류 0개
  - [ ] TODO(선택): `alignTangent` 파라미터 제거 리팩터링, `AlignmentPlacementSupportTests`에 세그먼트 시작/끝점 검증 추가 — 사용자 승인 후 진행
  - [ ] TODO: Revit 실제 배치 결과(6m마다 끊긴 연속 보) 육안 검증
...
### 관로 네트워크 — 분기(T/십자) 및 곡관 자동 배치 (2026-07-30 기획 착수)
SHP 각 레코드 = 폴리선 1개. 폴리선 간 접점/교차점에는 이형관(티·크로스)을, 폴리선이 꺾이는
절점에는 곡관을 배치해야 한다. 현재 `AlignmentIntervalSampler`는 단일 폴리선의 등간격 분절만
수행하며 **폴리선 사이의 위상(topology) 개념이 전혀 없다**.

> ①② 완료(2026-07-30, 위 "완료된 작업" 참조). ③④⑤ 지시서 = `docs/20_관로네트워크_분기곡관_지시서.md`

- [x] **1단계 — 위상 그래프 구축** (`Core/Gis/PipeNetworkGraph`)
  - 전체 폴리선의 정점/끝점을 스냅 허용오차로 병합해 노드 생성 (사용자 결정에 따라 **3D 거리 기준**)
  - 끝점이 다른 폴리선의 **중간(선분 위)** 에 놓인 경우 → 해당 폴리선을 그 지점에서 분할(split)
  - 폴리선끼리 교차 → 교차점에서 양쪽 분할. 평행 중첩은 미처리(TODO)
  - 노드 차수(degree) 산정: 1=단부, 2=직선/곡관 후보, 3=T분기, 4=십자, 5+=경고
- [x] **2단계 — 노드 타입 판정** (차수 + 직경 조합 → 부재 결정)
  - 3차수: 주관/지관 직경 비교 → 티 / 이경티 (**분기각이 직각이 아닐 때의 경고는 미구현** — 배치 단계로 이월)
  - 4차수: 크로스. 1차수: 맹판/캡. 2차수 + 직경 다름: 레듀서
- [ ] **3단계 — 곡관 판정** (`Core/Gis/BendResolver`)
  - 차수 2 노드에서 이전 선분·다음 선분의 편각 θ 계산(평면각/입체각 구분 필요)
  - 표준 곡관 각도 {11.25, 22.5, 45, 90}, 각 각도는 **직경별 허용굴곡 ±δ(D)** 를 가짐
  - `|θ − a| ≤ δ(D)` 인 표준각 a가 있으면 그 곡관 배치, θ ≤ δ(D) 면 곡관 없음(직관 허용편차 흡수)
  - 어느 범위에도 안 들어가면 **미해결(Unresolved)** 로 표시만 하고 배치는 보류
- [ ] **4단계 — 진단 리포트 UI**: 미해결 절점/과다 차수 노드를 목록으로 보여주고 해당 위치로 뷰 이동
- [ ] **5단계 — Revit 배치**: 이형관·곡관 패밀리 배치 + 직관 구간 재분절(부재 길이를 이형관 몸통만큼 축소)

#### ⚠️ 추후 고민 — 표준각 범위 밖 편각 처리
> 예: 편각 16°. 11.25° 곡관의 허용굴곡 범위, 22.5° 곡관의 허용굴곡 범위 어디에도 들어가지 않는다.
> 이 경우 **곡관 여러 개를 조합**(예: 11.25° + 잔여 편각을 직관 허용편차로 흡수, 또는 11.25°×2에
> 짧은 직관 삽입)하거나 현장 절단/특수 곡관으로 처리해야 한다.
> 조합 시 곡관 사이 최소 직관 길이, 조합 결과의 중심선이 원래 폴리선과 벌어지는 오차 허용치,
> 어느 조합을 우선할지(부재 수 최소 vs 오차 최소)를 정해야 한다. **1~5단계와 분리해 별도 결정.**
> 그때까지는 3단계에서 Unresolved로 표시만 하고 곡관을 넣지 않는다.

#### ✅ 확정 사항 (2026-07-30 사용자 결정)
- **1차 범위 = ①~④** (위상 그래프 → 노드 판정 → 곡관 판정 → 진단 리포트). ⑤ Revit 배치는 다음 차수.
- **허용굴곡 δ 출처 = 설정창 + DataStorage.** 사용자가 관종/직경별 δ를 입력·저장한다.
  거푸집 Spec 설정창과 동일한 패턴(Schema 클래스 분리 + `DataStorage`)을 따른다.
- **편각 θ = 3D 입체각 하나로 계산**(이전/다음 선분 벡터의 사잇각). 수평/수직 분리 안 함.
  → ⑤ 배치 단계에서 곡관의 회전각(roll)을 이 벡터쌍의 법선에서 유도해야 함. 배치 시 재확인 필요.
- **노드 스냅 허용오차 = 기본 10mm, UI에서 조정 가능.**

#### ⚠️ 남은 결정 (⑤ 배치 착수 시점)
- 이형관(티/이경티/크로스)·곡관 RFA 존재 여부 및 파라미터 규약
- 이형관 몸통 길이만큼 직관 구간을 축소하는 재분절 규칙
...
### ValveRoomView 참고도 슬롯 축소 (평면 2→1, 단면 2→1) (2026-07-22)
- [x] `ValveRoomView.xaml` 우측 참고도 패널에서 평면도 Border 2개 → 1개, 단면도 Border 2개 → 1개로 축소.
  남긴 평면도 Border의 `Margin`을 `0,0,0,16`으로 조정해 "단면도" 라벨 앞 여백 유지.
- [x] `ValveRoomViewModel`: `PlanImagePath1/2`, `SectionImagePath1/2` → `PlanImagePath`, `SectionImagePath`로 통합.
  참조 파일명도 `{종류}_평면도.png`, `{종류}_단면도.png`로 변경(`1` 접미사 제거). `SelectedValveRoomType` setter의
  `OnPropertyChanged` 4줄 → 2줄, 150행 TODO 주석 예시 파일명도 갱신.
- [x] 검증: `dotnet build src/DHBIMWATER.UI/DHBIMWATER.UI.csproj -p:DebugType=none` 오류 0 (XAML 바인딩 포함).
  Revit(PID 18204) 실행 중이라 `DHBIMWATER.Core.pdb` 잠금 → pdb 생성 끄고 컴파일만 검증.
- [ ] TODO: Revit 닫고 정상 빌드(pdb 갱신) 1회. 참고도 이미지가 아직 없어 화면은 회색 빈 칸으로 표시됨 — 실제 레이아웃/여백 육안 확인 필요.
