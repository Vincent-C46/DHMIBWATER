...
public sealed class ModelPipeAlignmentUseCase
{
    ...
    { _transaction = transaction; _loader = loader; _alignmentRepo = alignmentRepo; _straightRepo = straightRepo; _pipeRepo = pipeRepo; _projectLocationRepo = projectLocationRepo; _sharedParameterRepo = sharedParameterRepo; _bendSettingsRepo = bendSettingsRepo; _adaptiveBendRepo = adaptiveBendRepo; }
    ...
    public PipeAlignmentModelingResult Execute(PipeAlignmentModelingRequest request)
    {
        ...
        var storedSettings = _bendSettingsRepo.Load();
        var settings = storedSettings ?? BendSettings.Default;
        ...
        var (bendPlan, bendWarnings) = request.OutputMode == PipeAlignmentOutputMode.Adaptive
            ? PlanBends(loaded.Alignments, request, settings, storedSettings is null)
            : (null, Array.Empty<string>());

        // 트랜잭션을 열기 전에 카탈로그가 가리키는 곡관 패밀리가 실제로 이 문서에 로드돼 있는지 확인한다.
        // 그렇지 않으면 직관 배치까지 다 끝낸 뒤 곡관 배치 단계에서야 실패해 전체가 롤백된다.
        if (bendPlan is not null) EnsureBendFamiliesLoaded(bendPlan, request.BendFamilyName);
        ...
        using (_transaction)
        {
            try
            {
                ...
                var placementWarnings = request.OutputMode == PipeAlignmentOutputMode.Adaptive && bendPlan is not null
                    ? PlaceBendFittings(bendPlan, origin, request)
                    : Array.Empty<string>();
                _transaction.Commit();
    ...
    private (BendTrimPlan? Plan, IReadOnlyList<string> Warnings) PlanBends(IReadOnlyList<PipeAlignment> alignments, PipeAlignmentModelingRequest request, BendSettings settings, bool usesDefault)
    {
        ...
        if (usesDefault) warnings.Add("저장된 관로 규격 설정이 없어 빈 기본값으로 판정했습니다. 모든 곡관 절점은 설정 누락으로 표시됩니다.");
    ...
    private IReadOnlyList<string> PlaceBendFittings(BendTrimPlan plan, AlignmentPlacementOrigin origin, PipeAlignmentModelingRequest request)
    {
        ...
        var bendPlans = placeable.Select(x => new AdaptiveBendPlacementPlan(
            ...
            request.BendDiameterParameterName, request.BendWallThicknessParameterName, x.IsAcceptable,
