...
public sealed class PlaceAlignmentFamilyUseCase
{
    ...
    public PlaceAlignmentFamilyUseCase(ITransactionContext transaction, IEnumerable<IAlignmentSourceReader> readers, IAlignmentBeamPlacementRepo beamRepo, IAlignmentPipePlacementRepo pipeRepo)
    { _transaction = transaction; _readers = readers.ToList(); _beamRepo = beamRepo; _pipeRepo = pipeRepo; }
    ...
    public AlignmentFamilyPlacementResult Execute(AlignmentFamilyPlacementRequest request)
    {
        ...
        using (_transaction)
        {
            try
            {
                ...
                {
                    AlignmentPlacementTarget.Beam => _beamRepo.PlaceAlong(alignments, Require(request.BeamTypeName, "빔 유형"), request.LevelName, request.IntervalMm / 1000d, request.AlignTangent),
                    AlignmentPlacementTarget.PipingSystem => _pipeRepo.PlaceAlong(alignments, Require(request.PipingSystemTypeName, "파이프 시스템 유형"), Require(request.PipeTypeName, "PipeType"), request.LevelName, request.IntervalMm / 1000d),
