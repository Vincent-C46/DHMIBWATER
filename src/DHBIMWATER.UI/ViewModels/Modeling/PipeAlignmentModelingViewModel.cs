...
public sealed class PipeAlignmentModelingViewModel : ViewModelBase
{
    ...
    private double _referenceX, _referenceY, _referenceZ, _intervalM = 6, _snapToleranceMm = 10;
    ...
    public double ReferenceX { get => _referenceX; set => SetProperty(ref _referenceX, value); } public double ReferenceY { get => _referenceY; set => SetProperty(ref _referenceY, value); } public double ReferenceZ { get => _referenceZ; set => SetProperty(ref _referenceZ, value); }
    ...
    public string ProjectionDetails => SelectedFile is null ? string.Empty : string.IsNullOrWhiteSpace(SelectedFile.ReadResult.ProjectionWkt) ? "좌표계 정보 없음 (DXF)" : $"추천: {SelectedFile.ReadResult.RecommendedEpsg ?? "없음"}\n{SelectedFile.ReadResult.ProjectionWkt}";
    ...
    private void AddFile() { var path = _fileDialog.OpenFile("관로 선형 파일 선택", "관로 선형 파일 (*.shp;*.dxf)|*.shp;*.dxf|모든 파일 (*.*)|*.*"); if (string.IsNullOrWhiteSpace(path) || Files.Any(x => string.Equals(x.Path, path, StringComparison.OrdinalIgnoreCase))) return; try { var reader = _readers.FirstOrDefault(x => x.CanRead(path)) ?? throw new InvalidOperationException($"'{path}' 파일을 읽을 수 있는 리더가 없습니다."); var read = reader.Read(path); var item = new PipeAlignmentSourceFileItem { Path = path, ReadResult = read, MappingChanged = NotifyFileMappingChanged }; item.DiameterField = AlignmentAttributeParser.GuessDiameterField(read.Fields.Select(x => x.Name)); item.KindField = AlignmentAttributeParser.GuessKindField(read.Fields.Select(x => x.Name)); Files.Add(item); SelectedFile = item; RefreshReferencePoint(); NotifyFileMappingChanged(); } catch (Exception ex) { _dialog.Warn("파일 읽기 실패", ex.Message); } }
    ...
    private void RefreshReferencePoint() { var point = AlignmentReferencePoint.FromFirstVertex(Files.SelectMany(x => x.ReadResult.Features)); ReferenceX = point?.X ?? 0; ReferenceY = point?.Y ?? 0; ReferenceZ = point?.Z ?? 0; }
    private IReadOnlyList<AlignmentSourceFile> SourceFiles() => Files.Select(x => new AlignmentSourceFile(x.Path, x.PipeKind, x.DiameterField, x.KindField)).ToList();
